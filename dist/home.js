...
function initHeroSection() {
    ...
    const tl = gsap.timeline({
        paused: true,
        onComplete: ()=>{
            if (canUseDOM) window.sessionStorage.setItem("heroIntroPlayed", "true");
        }
    });
    // Animation sequence --------------------------------------------
    ...
    }, "-=1.5").to(".section-reels", {
        marginTop: "-12rem",
        duration: 0.5,
        ease: "power4.Out"
    }, "-=0.5");
    // Check if intro was already played this session
    const hasSeenIntro = canUseDOM && window.sessionStorage.getItem("heroIntroPlayed") === "true";
    // Play or skip
    if (hasSeenIntro) // Jump straight to end state
    tl.progress(1);
    else // First visit: delay then play
